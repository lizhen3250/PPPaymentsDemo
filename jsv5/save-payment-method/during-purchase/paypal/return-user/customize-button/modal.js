...
selectElement.addEventListener('change', function () {
    ...
    if (currentGoodsType == 'physical') {
        if (selectElement.value == 'Password Free') {
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_initiator = "CUSTOMER";
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_type = "ONE_TIME";
            } else if (selectElement.value == 'Recurring Payment') {
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_initiator = "MERCHANT";
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_type = "RECURRING";
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.usage = "SUBSEQUENT";
            }
    } else if (currentGoodsType == 'virtual') {
        if (selectElement.value == 'Password Free') {
                virtualGoodsTemplates[0].payment_source.paypal.stored_credential.payment_initiator = "CUSTOMER";
                virtualGoodsTemplates[0].payment_source.paypal.stored_credential.payment_type = "ONE_TIME";
            } else if (selectElement.value == 'Recurring Payment') {
                virtualGoodsTemplates[0].payment_source.paypal.stored_credential.payment_initiator = "MERCHANT";
                virtualGoodsTemplates[0].payment_source.paypal.stored_credential.payment_type = "RECURRING";
                virtualGoodsTemplates[0].payment_source.paypal.stored_credential.usage = "SUBSEQUENT";
            }
    }
...
function updateGoodsType(type) {
    ...
    if (type === "physical") {
        ...
        if (selectElement.value == 'Password Free') {
                console.log("password free inside");
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_initiator = "CUSTOMER";
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_type = "ONE_TIME";
            } else if (selectElement.value == 'Recurring Payment') {
                console.log("recurring payment inside");
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_initiator = "MERCHANT";
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.payment_type = "RECURRING";
                physicalGoodsTemplates[0].payment_source.paypal.stored_credential.usage = "SUBSEQUENT";
            }
